 ...
 */
declare class RnBridgeApi {
  /**
   * 监听 Native 消息
   * @param {*} params -{event: '事件名', success: '回调函数', tag: '事件标识', type: '事件类型1单次2普通'}
   */
  static eventListener(params: {
    event: string;
   ...
   * ❗️移除长监听
   */
  static eventRemove(key: string, tag?: string): void;
   ...
   * 显示分享面板
   * @param title -显示面板标题
   * @param items -[{tag: '分享类型', link: '链接', url: '分享地址', path:'小程序路径', image: '分享Icon图片', title:'分享标题‘, openId:'小程序原始id', desc:'分享描述', scene:'场景', ticket:'', env:'版本环境' }]
  ...
  /**
   * 更新分享面板数据
   * @param params -items[{tag: '分享类型', url: '分享地址', path:'小程序路径', image: '分享Icon图片', title:'分享标题‘, openId:'小程序原始id', desc:'分享描述', scene:'场景', ticket:'', env:'版本环境' }]
   */
  static shareData(items: Array<ShareParams>): Promise<any>;
   ...
   * @returns
   */
  static chooseDate(params?: any): Promise<any>;
   ...
   * @returns
   */
  static chooseFile(params?: any): Promise<any>;
   ...
   * @returns
   */
  static openFile(params?: any): Promise<any>;
  /**
   * 预览图片
