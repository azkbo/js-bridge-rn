...
class RnBridgeApi {
   ...
   */
  static eventListener({ event = null, success = null, tag = '', type = 2 } = {}) {
  ...
  /**
   * 显示分享面板
   * @param params -items[{tag: '微信分享类型', link: '链接', url: '分享地址', path:'小程序路径', image: '分享Icon图片', title:'分享标题‘, openId:'小程序原始id', desc:'分享描述', scene:'场景', ticket:'', env:'版本环境' }]
   * @param tag -微信分享类型: mini:小程序，web:h5，video:视频，image:图片，file:文件，text:文本内容，
  ...
  /**
   * 更新分享面板数据
   * @param params -items[{tag: '微信分享类型', url: '分享地址', path:'小程序路径', image: '分享Icon图片', title:'分享标题‘, openId:'小程序原始id', desc:'分享描述', scene:'场景', ticket:'', env:'版本环境' }]
   */
  static shareData(params = {}) {
    return postEventToNative({ event: 'shareData', params });
  }
  ...
  static setHeaderConfig(params = {}) {
    return postEventToNative({ event: 'headerConfig', params });
  }
  ...
  /**
   * 顶部右边菜单栏
   * @param params -{item: [{icon: '图标', name: '操作名', tag: '标识', style:{color: string, fontSize: number}}]}
   */
   ...
   * @returns 
   */
  static chooseFile(params = {}) {
    return postEventToNative({ event: 'chooseFile', params });
  }
   ...
   * @returns 
   */
  static openFile(params = {}) {
    return postEventToNative({ event: 'openFile', params });
  }
